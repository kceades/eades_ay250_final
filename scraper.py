...
class Scraper(object):
	...
	def pull_data(self,save_name='pubmeta'):
		...
		:save_name: (str) the name to save the pickled metadata to
		"""
		self.data = {}
		self.sns = set()
		for url in tqdm(self.urls):
			self.pull_url_data(url)
		if self.data!={} and len(self.sns)>0:
	...
	def pull_url_data(self,url):
		...
		:returns: (bool) False if the given url does not have the requisite data
